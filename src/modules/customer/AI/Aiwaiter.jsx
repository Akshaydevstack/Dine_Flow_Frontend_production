  ...
  return (
    ...
    >
      ...
      <div className="p-2.5 flex flex-col gap-2 flex-1">
        <div>
          <h4 className="text-[12px] font-semibold text-slate-800 dark:text-white leading-tight line-clamp-2">
            {dish.name}
          </h4>
          <p className="text-[12px] font-bold mt-0.5 text-violet-600 dark:text-violet-400">
            ₹{dish.price}
          </p>
        </div>
  ...
  return (
    ...
    >
      <img
        src={
          item.image ||
          "https://res.cloudinary.com/dxsimc9dz/image/upload/v1738734327/placeholder.jpg"
        }
        alt={item.name}
        className="w-10 h-10 rounded-xl object-cover flex-shrink-0 bg-slate-100 dark:bg-[#0e1220]"
      />
      ...
      <div className="flex items-center rounded-xl overflow-hidden flex-shrink-0 bg-slate-50 border border-indigo-100 dark:bg-[#0e1220] dark:border-white/5">
        ...
        <span className="text-[12px] font-bold w-6 text-center text-slate-800 dark:text-white">
          {item.quantity}
        </span>
        <button
          onClick={(e) => {
...
function CartCard({ cart, onSend, onNavigate }) {
  const total = cart.reduce(
    (sum, item) => sum + parseFloat(item.total || item.price * item.quantity),
    0,
  );
  return (
...
function PastOrderCard({ order, onSend, onNavigate }) {
  ...
  const statusMap = {
    ...
    READY:
      "text-violet-600 bg-violet-50 border-violet-200 dark:text-violet-400 dark:bg-violet-500/10 dark:border-violet-500/30",
    COMPLETED:
      "text-slate-600 bg-slate-50 border-slate-200 dark:text-slate-400 dark:bg-slate-500/10 dark:border-slate-500/30",
    CANCELLED:
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden animate-slide-up transition-transform bg-white/80 dark:bg-[#1a2035]/90 border border-indigo-100 dark:border-white/5 backdrop-blur-md shadow-sm hover:shadow-md">
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full font-medium border ${statusClasses}`}
          >
            {status}
          </span>
        </div>
      ...
      <div className="max-h-[160px] overflow-y-auto hide-scrollbar bg-white/40 dark:bg-black/10">
        {itemsList.map((item, i) => (
          <div
            key={i}
            className="flex items-center gap-3 px-4 py-2 border-b border-indigo-50 dark:border-white/5 last:border-0"
          >
            {item.image ? (
              <img
                src={item.image}
                alt={item.name}
                className="w-8 h-8 rounded-lg object-cover flex-shrink-0 bg-slate-100 dark:bg-[#0e1220]"
              />
            ) : (
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs flex-shrink-0 bg-slate-100 dark:bg-[#0e1220] opacity-50">
                🍽️
              </div>
            )}
            <div className="flex-1 min-w-0">
      ...
      <div className="px-4 py-3 border-t border-indigo-100 dark:border-white/5">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              onNavigate(`/customer/orders/${order.order_id}`);
            }}
    ...
    >
      ...
      <div className="max-h-[160px] overflow-y-auto hide-scrollbar bg-white/40 dark:bg-black/10">
        {itemsList.map((item, i) => (
          <div
            key={i}
            className="flex items-center gap-3 px-4 py-2 border-b border-emerald-50 dark:border-white/5 last:border-0"
          >
            {item.image ? (
              <img
                src={item.image}
...
function Message({ msg, isAnimating, animatedText, onNavigate, onSend }) {
  ...
  const content =
    isAnimating && animatedText !== undefined ? animatedText : msg.content;
  ...
  return (
    <div className="flex gap-3 mb-5 animate-slide-up">
      ...
      <div className="flex flex-col gap-3 min-w-0 flex-1">
        ...
        {showCards && msg.cart?.length > 0 && (
          <CartCard cart={msg.cart} onSend={onSend} onNavigate={onNavigate} />
        )}
        {showCards && msg.current_order && (
          <OrderCard order={msg.current_order} onNavigate={onNavigate} />
        )}
  ...
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center px-6 bg-slate-900/20 dark:bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-xs rounded-2xl p-5 animate-scale-in bg-white/95 dark:bg-[#161b2e]/95 border border-indigo-100 dark:border-white/10 shadow-xl">
        ...
        <p className="text-[13px] mb-5 text-slate-500 dark:text-slate-400">
...
export default function AiWaiter({ onClose }) {
  ...
  const idRef = useRef(Math.max(0, ...messages.map((m) => m.id ?? 0)) + 1);

  const now = () =>
    new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });

  useEffect(() => {
    saveMessages(tableId, messages);
  }, [messages, tableId]);
  ...
  const send = useCallback(
    async (text) => {
      ...
      // 📍 FETCH LOCATION BEFORE SENDING (Using the 2-Stage Strategy)
      let userLat = null;
      let userLng = null;

      try {
        // This will now resolve instantly on Android if low-accuracy is available
      ...
      try {
        ...
        const msg = {
          ...
          bottom_text: data.bottom_text || null,
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between px-4 pt-6 pb-2.5 flex-shrink-0 border-b border-indigo-100 dark:border-white/5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-[14px] font-bold text-slate-800 dark:text-white leading-tight">
                  AI Waiter
                </h2>
                <p className="text-[10px] mt-0.5 text-slate-500 dark:text-slate-400">
                  {restaurant_name
                    ? `${restaurant_name} · Table ${table_number || tableId}`
                    : "Always here to help"}
                </p>
              </div>
            </div>
          ...
          {/* Messages */}
          <div
            className="relative z-10 flex-1 overflow-y-auto px-4 py-4 hide-scrollbar overscroll-contain"
            style={{ WebkitOverflowScrolling: "touch" }}
          >
            {messages.map((msg) => (
              <Message
          ...
          {/* Input */}
          <div
            className="relative z-10 px-3 pt-2 flex-shrink-0 border-t border-indigo-50 dark:border-white/5"
            style={{
